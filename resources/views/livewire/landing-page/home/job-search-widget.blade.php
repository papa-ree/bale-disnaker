...
@if($section)
    @php
        ...
        $buttonLabel = $buttons[0]['label'] ?? 'Cari Lowongan';
    @endphp
                        ...
                        <div class="flex flex-col lg:flex-row gap-4">
                            <div class="flex-1 items-center relative">
                                <input type="text" wire:model="keyword"
                                    placeholder="{{ $section->meta('custom.search_placeholder', 'Cari lowongan kerja...') }}"
                                ...
                                @foreach ($this->latestJobs as $latestJob)
                                    <a href="{{ route('bale.view-job', $latestJob->slug) }}" wire:navigate.hover
                                        ...
                                        <div class="flex items-center gap-4 min-w-0">
                                            ...
                                            <div class="min-w-0">
                                                <p
                                                    class="font-semibold text-gray-900 dark:text-white text-sm truncate group-hover:text-teal-700 dark:group-hover:text-teal-400 transition-colors">
                                                    {{ $latestJob->nama_pekerjaan }}
                                                </p>
                                                <div
                                                    class="flex items-center gap-3 mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                                        ...
                                        <div class="flex items-center gap-3 shrink-0 ml-4">
                                            @if ($latestJob->tipe)
                                                <span
                                                    class="hidden sm:inline-flex px-2.5 py-1 bg-teal-50 dark:bg-teal-900/20 text-teal-700 dark:text-teal-400 rounded-full text-xs font-medium capitalize">
                                                    {{ $latestJob->tipe }}
                                                </span>
                                            @endif
                                            ...
                                            <x-lucide-chevron-right
                                                class="w-4 h-4 text-gray-300 dark:text-gray-600 group-hover:text-teal-500 transition-colors" />
                                        </div>
                                    </a>
                                @endforeach
                            </div>
...
@else
    <x-umpak::section-error name="Widget Pencarian Kerja" />
@endif