...
<footer class="bg-gray-950 text-gray-300 pt-20 pb-10">
    <div class="container mx-auto px-4">
        @if (!$footerSection)
            <x-umpak::section-error title="Konten Footer Tidak Ditemukan"
                message="Silakan konfigurasi section 'footer-section' di panel admin CMS." />
        @else
            @php
                $aboutMeta = $heroSection?->meta ?? [];
                        ...
                        <ul class="space-y-3">
                            @if(!empty($contact['address']))
                                <li class="flex items-start gap-3">
                                    <x-umpak::icon name="map-pin" class="w-[18px] h-[18px] text-teal-400 mt-1 shrink-0" />
                                    <span class="text-sm">{{ $contact['address'] }}</span>
                                </li>
                            @endif
                            @if(!empty($contact['phone']))
                                <li class="flex items-center gap-3">
                                    <x-umpak::icon name="phone" class="w-[18px] h-[18px] text-teal-400 shrink-0" />
                                    <span class="text-sm">{{ $contact['phone'] }}</span>
                                </li>
                            @endif
                            @if(!empty($contact['email']))
                                <li class="flex items-center gap-3">
                                    <x-umpak::icon name="mail" class="w-[18px] h-[18px] text-teal-400 shrink-0" />
                            ...
                            @foreach($socials as $social)
                                <a href="{{ $social['url'] }}" target="_blank" rel="noopener noreferrer"
                                    ...
                                    <span
                                        ...
                                        @if($social['icon'])
                                            {!! \Bale\Umpak\Support\Sanitizer::cleanSvg($social['icon']) !!}
                                        @else
                                            <x-umpak::icon name="link" class="w-[18px] h-[18px]" />
                                        @endif
                                    </span>
                                </a>
                            @endforeach
